...
logger = get_logger("live2d_model", 'orange')
...
class Live2DModel(QOpenGLWidget):
    ...
    def __init__(self):
        ...
        logger.info("初始化Live2D模型控制器... [ 进行中 ]")
    ...
    def initializeGL(self):
        ...
        try:
            ...
            if not model_loaded and hasattr(live2d, 'LIVE2D_VERSION') and live2d.LIVE2D_VERSION == 3:
                # 尝试加载2D文件夹中的模型
                for root, dirs, files in os.walk("data/plugins/kira_live2d/2D"):
                    for file in files:
